...
31-ts-lstm.py
-------------
Sequence-to-one time series regression using a single-layer LSTM.
Loads training/testing CSVs from pytorch/data. Includes simple
early-stopping-like logic based on loss improvements.
"""
...
if __name__ == "__main__":
  ...
  TRAIN_MODE = True
  if TRAIN_MODE:
    model = create_torch_lstm(n_neurons=2, look_back=2)
    model = train_torch_lstm(model, df_train, n_epochs=3000, lr=0.001)

  preds = predict_torch_lstm(model, df_test)
  print(preds)
