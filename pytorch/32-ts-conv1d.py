...
32-ts-conv1d.py
---------------
1D CNN for time series regression on sliding-window data.
Demonstrates feature extraction via Conv1d followed by a small MLP head.
Loads training/testing CSVs from pytorch/data.
"""
...
if __name__ == "__main__":
  ...
  df_train = pd.read_csv(data_dir / 'data_ts_train.csv')
  df_test = pd.read_csv(data_dir / 'data_ts_test.csv')

  TRAIN_MODE = True
  if TRAIN_MODE:
    model = create_torch_conv1d(in_channels=1, input_dim=2)
    model = train_torch_conv1d(model, df_train, n_epochs=3000, lr=0.001)

  preds = predict_torch_conv1d(model, df_test)
  print(preds)
